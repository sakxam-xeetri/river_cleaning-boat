/*
 * ==================================================================================
 * ESP8266 RIVER CLEANING BOAT FIRMWARE - PLATFORMIO MAIN
 * BY HIMALIXLABS
 * ----------------------------------------------------------------------------------
...
// Function Prototypes for PlatformIO C++ Compilation
void stopMotors();
...
// ----------------------------------------------------------------------------------
// EMBEDDED WEB CONTROLLER HTML (HIMALIXLABS CONTROLLER UI)
// ----------------------------------------------------------------------------------
...
void loop() {
  ...
  if (currentDirection != "STOP" &&
      (millis() - lastCommandTime > FAILSAFE_TIMEOUT_MS)) {
    Serial.println(
        "[SAFETY] Fail-safe timeout triggered! Auto-stopping motors.");
    stopMotors();
  }
}
